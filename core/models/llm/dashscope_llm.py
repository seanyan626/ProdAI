...
def _convert_dict_messages_to_dashscope(messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    # DashScope 的格式与 OpenAI 基本兼容，这里主要是为了确保 role 的有效性
    # 并且可以作为一个适配层，以防未来格式发生变化。
...
class DashScopeLLM(BaseLLM):
        ...
        **kwargs: Any
        ...
        try:
            ...
            response: GenerationResponse = dashscope.Generation.call(
                ...
                result_format='message',  # 获取 message 格式的输出
                **api_params
            )
    ...
    async def achat(
        ...
        **kwargs: Any
        ...
        真正的异步需要使用 aiohttp 或 httpx 等库自己封装异步请求，或等待官方SDK支持。
        这里我们用 asyncio.to_thread 模拟异步调用。
        """
        import asyncio

        # 同步版本的 chat 方法已经很完善，我们直接在线程中运行它
        # 这不是真正的I/O异步，但能防止阻塞主事件循环
        try:
