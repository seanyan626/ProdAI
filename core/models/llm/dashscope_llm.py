...
# 阿里云通义千问 (DashScope) LLM 接口实现 (使用 Langchain 封装)
import logging
...
class DashScopeLLM(BaseLLM):
    """
    使用 Langchain 封装通义千问系列模型。
    由于其 API 与 OpenAI 兼容，我们复用 ChatOpenAI 客户端。
    """
    def __init__(
        self,
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            logger.info(f"Langchain ChatOpenAI (用于 DashScope) 客户端已为模型 {self.model_name} 初始化。Base URL: {self.config.get('base_url') or '默认'}")
        except Exception as e:
            logger.error(f"初始化 Langchain ChatOpenAI (用于 DashScope) 客户端失败: {e}", exc_info=True)
            raise
    ...
    def generate(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.error(f"DashScope LLM generate 调用失败: {e}", exc_info=True)
    ...
    async def agenerate(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.error(f"DashScope LLM agenerate 调用失败: {e}", exc_info=True)
    ...
    def chat(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.error(f"DashScope LLM chat 调用失败: {e}", exc_info=True)
    ...
    async def achat(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.error(f"DashScope LLM achat 调用失败: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    if not DASHSCOPE_API_KEY or DASHSCOPE_API_KEY == "YOUR_DASHSCOPE_API_KEY_HERE":
        ...
        logger.info("测试 DashScopeLLM (Langchain 封装版)...")
        try:
            llm = DashScopeLLM()
            logger.info(f"LLM 实例创建成功: {llm.get_model_info()}")

            logger.info("\n--- 测试同步 Chat ---")
            chat_messages = [{"role": "user", "content": "你好，用中文介绍一下你自己。"}]
